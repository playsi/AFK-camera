...
public class AFKcamResourceReloadListener implements SimpleResourceReloadListener<List<RawAnimation>> {
    ...
    private static final Identifier LISTENER_ID = new Identifier("cinematic", "camera_reload");
    ...
    @Override
    public CompletableFuture<List<RawAnimation>> load(ResourceManager manager, Profiler profiler, Executor executor) {
        return CompletableFuture.supplyAsync(() -> ANIM_SERVICE.onLoadRPs(manager), executor);
    }

    @Override
    public CompletableFuture<Void> apply(List<RawAnimation> animations, ResourceManager manager, Profiler profiler, Executor executor) {
